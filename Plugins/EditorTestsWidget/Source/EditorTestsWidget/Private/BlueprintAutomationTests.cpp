...
DEFINE_LOG_CATEGORY_STATIC(LogWidgetAutomationTests, Log, All);

IMPLEMENT_COMPLEX_AUTOMATION_TEST(FCompileWidgetBlueprintsTest, "Project.Blueprints.Compile Widgets", EAutomationTestFlags::EditorContext | EAutomationTestFlags::StressFilter)
...
public:
	...
	static void CollectTestsByClass(UClass * Class, TArray< FString >& OutBeautifiedNames, TArray< FString >& OutTestCommands, bool bIgnoreLoaded) 
	{
		...
		AssetRegistryModule.Get().GetAssetsByClass(Class->GetClassPathName(), ObjectList, true);
	...
	static void CloseBlueprint(UBlueprint* const BlueprintObj)
	{
		...
		{
			UE_LOG(LogWidgetAutomationTests, Log, TEXT("Closing '%s' so we don't invalidate the open version when unloading it."), *BlueprintObj->GetName());
	...
	static void UnloadBlueprint(UBlueprint* const BlueprintObj, bool bForceFlush = false)
	{
		...
		{
			UE_LOG(LogWidgetAutomationTests, Log, TEXT("No need to unload '%s' from the transient package."), *BlueprintObj->GetName());
		}
		else if (OldPackage->IsRooted() || BlueprintObj->IsRooted())
		{
			UE_LOG(LogWidgetAutomationTests, Error, TEXT("Cannot unload '%s' when its root is set (it will not be garbage collected, leaving it in an erroneous state)."), *OldPackage->GetName());
		}
		else if (OldPackage->IsDirty())
		{
			UE_LOG(LogWidgetAutomationTests, Error, TEXT("Cannot unload '%s' when it has unsaved changes (save the asset and then try again)."), *OldPackage->GetName());
		}
		else 
		{
	...
	static bool CompileBlueprint(const FString& BlueprintAssetPath)
	{
		...
		{
			UE_LOG(LogWidgetAutomationTests, Error, TEXT("Failed to compile invalid blueprint, or blueprint parent no longer exists."));
			return false;
		}
...
void FCompileWidgetBlueprintsTest::GetTests(TArray<FString>& OutBeautifiedNames, TArray <FString>& OutTestCommands) const
{
	FBlueprintAutomationTestUtilities::CollectTestsByClass(UUserWidgetBlueprint::StaticClass(), OutBeautifiedNames, OutTestCommands, false);
}

bool FCompileWidgetBlueprintsTest::RunTest(const FString& Parameters)
{
